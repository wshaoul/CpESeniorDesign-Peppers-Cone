# studio_circle.py
#
# Four-view Circle Hologram Studio
# Replaces studio_main.py with a version that uses the four-view circle hologram
# pipeline from live_view_circle.py instead of the Pepper's Cone warp.
...
# --- Import pages (circle editions) with placeholder fallbacks ---
try:
    from upload_view_circle import UploadView
except Exception:
    class UploadView(ttk.Frame):
        def __init__(self, parent, controller=None):
            super().__init__(parent)
            ttk.Label(self, text="Upload View (placeholder)",
...
try:
    from record_view_circle import RecordView
except Exception:
    class RecordView(ttk.Frame):
        def __init__(self, parent, controller=None):
            super().__init__(parent)
            ttk.Label(self, text="Record View (placeholder)",
                      font=("Segoe UI", 16, "bold")).pack(pady=40)
...
try:
    from live_view_circle import LiveView
except Exception:
    class LiveView(ttk.Frame):
        def __init__(self, parent, controller=None):
            super().__init__(parent)
            ttk.Label(self, text="Live View (placeholder)",
                      font=("Segoe UI", 16, "bold")).pack(pady=40)
...
APP_TITLE = "Circle Hologram Studio"
...
def make_card(parent, title=None, subtitle=None, pad=(20, 16), rounded=18):
    ...
    def draw(_=None):
        ...
        _create_round_rect(cnv, 10, 12, w - 6, h - 6, rounded + 2,
                           fill="#edf2ff", outline="#edf2ff", tags="bg")
        _create_round_rect(cnv, 6, 6, w - 10, h - 10, rounded,
                           fill="#ffffff", outline="#e5e7eb", tags="bg")
    ...
    if title:
        ...
        if subtitle:
            (tb.Label if TTKB else ttk.Label)(
                head, text=subtitle,
                **({"bootstyle": "secondary"} if TTKB else {})
            ).pack(side="right")
...
class ActionTile((tb.Frame if TTKB else ttk.Frame)):
    ...
    def __init__(self, parent, title, desc, emoji="🔴", command=None):
        ...
        icon_lbl = (tb.Label if TTKB else ttk.Label)(
            row, text=emoji, font=("Segoe UI Emoji", 30))
        ...
        self.card.bind("<Button-1>",      on_press)
        self.card.bind("<Return>",        on_press)
...
class PreviewCanvas(tk.Canvas):
    ...
    def _build_overlay(self):
        ...
        (tb.Label if TTKB else ttk.Label)(
            left, text="Ready",
            **({"bootstyle": "inverse-dark"} if TTKB else {})
        ).pack(side="left")
...
class HomePage(ttk.Frame if not TTKB else tb.Frame):
    """Dashboard — circle hologram edition."""
    def __init__(self, parent, controller):
        ...
        (tb.Label if TTKB else ttk.Label)(
            leftgrp, text="Circle Hologram Studio",
            font=("Segoe UI", 18, "bold")).pack(side="left")

        rightgrp = (tb.Frame if TTKB else ttk.Frame)(header); rightgrp.pack(side="right")
        (tb.Button if TTKB else ttk.Button)(
            rightgrp, text="Settings",
        ...
        tiles = [
            ActionTile(tile_grid, "Go Live", "Circle hologram live", "⭕",
        ...
        (tb.Label if TTKB else ttk.Label)(
            rgrid, text="Recent recordings",
            font=("", 10, "bold")).grid(row=0, column=0, sticky="w")
        (tb.Label if TTKB else ttk.Label)(
            rgrid, text="Uploads",
            font=("", 10, "bold")).grid(row=0, column=1, sticky="w", padx=(16, 0))
        (tb.Label if TTKB else ttk.Label)(
            rgrid, text="hi.mp4 — 2 min ago",
            **({"bootstyle": "secondary"} if TTKB else {})).grid(
                row=1, column=0, sticky="w", pady=(4, 0))
        (tb.Label if TTKB else ttk.Label)(
            rgrid, text="demo.mov — yesterday",
            **({"bootstyle": "secondary"} if TTKB else {})).grid(
                row=1, column=1, sticky="w", padx=(16, 0), pady=(4, 0))
        ...
        prev_card, prev = make_card(right, "Preview", "Circle hologram • 30 fps")
        ...
        def _select(label, value):
            ...
            (tb.Label if TTKB else ttk.Label)(
                col, text=label,
                **({"bootstyle": "secondary"} if TTKB else {})).pack(
                    anchor="w", pady=(0, 2))
            ent = (tb.Entry if TTKB else ttk.Entry)(col)
            ent.insert(0, value); ent.configure(state="readonly")
            ent.pack(fill="x")
        ...
        for i, (k, v) in enumerate([("Camera",     "Integrated (1080p)"),
            ...
            (tb.Label if TTKB else ttk.Label)(grid, text=v).grid(
        ...
        (tb.Button if TTKB else ttk.Button)(
            rightbtns, text="Start",
            **({"bootstyle": "dark"} if TTKB else {})).pack(side="left", padx=4)
        (tb.Button if TTKB else ttk.Button)(
            rightbtns, text="Stop",
            **({"bootstyle": "secondary-outline"} if TTKB else {})).pack(
                side="left", padx=4)
        (tb.Button if TTKB else ttk.Button)(
            rightbtns, text="Settings",
            **({"bootstyle": "secondary-outline"} if TTKB else {})).pack(
                side="left", padx=4)
