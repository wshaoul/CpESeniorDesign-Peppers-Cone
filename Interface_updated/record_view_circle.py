...
# The "Open Circle Screen (process now)" button plays the recorded file through
# the four-view circle hologram pipeline from live_view_circle.py.
...
class RecordView(ttk.Frame):
    """
    Record camera video with a live preview. The cone-screen button now plays
    the recorded file through the four-view circle hologram pipeline
    (matching the livefeed.py approach with a circular centre gap).
    """
    ...
    def __init__(self, parent, controller=None):
        ...
        ttk.Label(title_row, text="Record",
                  style="Header.TLabel").pack(side="left", padx=8)

        ttk.Label(
            left,
            text=("Choose a camera, set resolution/fps, and record. "
                  "When you're ready, click 'Open Circle Screen (process now)' "
                  "to play the recorded video through the four-view circle hologram "
                  "pipeline. Optionally save a warped copy while it plays."),
        ...
        self.cam_entry = ttk.Spinbox(row, from_=0, to=10, width=5,
                                      textvariable=self.cam_index)
        self.cam_entry.pack(side="left", padx=6)
        ttk.Button(row, text="Open",
                   command=self._open_camera).pack(side="left", padx=(6, 0))
        ttk.Button(row, text="Close",
                   command=self._close_camera).pack(side="left", padx=6)
        ...
        ttk.Entry(r3, textvariable=self.out_var).pack(
            side="left", fill="x", expand=True)
        ttk.Button(r3, text="Browse…",
                   command=self._choose_output).pack(side="left", padx=6)

        # Process mode
        proc_box = ttk.LabelFrame(left, text="Circle Screen Options")
    ...
    def _start_recording(self):
        ...
        if not self.writer or not self.writer.isOpened():
            messagebox.showerror(
                "Record",
                "Could not open VideoWriter. Try a different path or filename.")
            self.writer = None
            return
        ...
        self.record_thread = threading.Thread(
            target=self._record_loop, args=(w, h), daemon=True)
    ...
    # ---------- circle screen: process recorded video ON DEMAND ----------
    def _open_circle_screen_process_now(self):
        """
        Open a fullscreen OpenCV window that plays the recorded file through
        the four-view circle hologram pipeline in real time.
        """
        in_path = self.out_path or self.out_var.get().strip()
        if not in_path or not os.path.exists(in_path):
            messagebox.showwarning(
                "Circle Screen",
                "No recorded video found. Please record and stop first.")
            return
    ...
    def _circle_player_worker(self, in_path: str, save_copy: bool):
        ...
        # Optional writer (save circle-hologram copy while playing)
        writer    = None
        out_path  = None
        if save_copy:
    ...
    # ---------- progress helpers (kept for parity, unused in on-demand flow) ----------
    def _open_progress(self, out_path: str, total: int):
        try:
            self._progress = tk.Toplevel(self)
            self._progress.title("Processing…")
            self._progress.resizable(False, False)
            ttk.Label(self._progress,
                      text=f"Processing to {os.path.basename(out_path)}").pack(
                          padx=12, pady=(12, 6))
            self._pbar = ttk.Progressbar(self._progress, mode="determinate",
                                          length=320, maximum=max(1, total))
    ...
    def _tick_progress(self, value: int):
        if getattr(self, "_pbar", None) is not None and \
                getattr(self, "_progress", None) is not None:
            try:
